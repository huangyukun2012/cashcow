...
func Init(dbc *sql.DB) {
	...
	logSvc.ConfigDefaultLogger("/tmp/bilisou", "weixin", logging.INFO, logging.ROTATE_DAILY)
	logSvc.Serve()
	//defer logSvc.Stop()
...
func DownloadImage(url string, filename string) error {
	...
	file, err := os.Create("imgpool1/" + filename + ".jpg")
	if err != nil {
...
func CrawlBlog() {
	for true {
		...
		var categories = []int{}
		sql := "select keyword, category from blogseed where flag = 0"
